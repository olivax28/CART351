...
def launchIntro():
    ...
    entrySelect = input()
    #Sabine: edited so that it it not case sensitive
    #This is the list of all the potential entries to choose
...
#All entries stored as individual functions
def October1st():
...
def October5th():
    #Sets up the table with the coordinates
...
#To be used in the october 15th function to direct user to use only number 0 to 20
...
def October15th():
    ...
    #takes input from the user to be later passed through the checkInput and to use them as the index to select an entry
    number = input()
...
def October19th():
    ...
    table.add_column("Lat", justify = "center", style = "green")
    table.add_column("Long", justify = "center", style = "green")
     # depending on the numbers present in each data entry, a different ASCII art replaces it
    AQI_art = art("woman")
    AQI_art02 = art("eye")
    UID_art = art("shy")
    UID_art02 = art("Real Face")
    #Draws the ASCII art table
    for item in responseData:
        aqi = item["aqi"]
        uid = item["uid"]
        if "4" in aqi:
...
def October31st():
 ...
 #Appears after the pasword is inputed
def launchGuide():
...
#this is how the program begins, requests a password before launch (password is terminal)
